...
// A simple read-only star display
...
export default function SingleBlog() {
    ...
    const loadReviews = useCallback(async () => {
        if (!slug) return
        try {
            const data = await getBlogReviews(slug, 1, 4) // fetch only top 4 reviews
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                ...
                <article className="xl:col-span-8 space-y-6">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">{post.reviewCount} reviews</span>
                    </div>
                    ...
                    <div className="text-sm text-muted-foreground">
                        {post.author} · {new Date(post.date).toLocaleDateString()} · {post.views} views
                    </div>

                    <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: post.content }} />
                </article>

                {/* Sidebar: Reviews + Add Review */}
